...
export default async function PsychologistsList() {
  const psychologistsList = await client.api.users.psychologists.$get(
    undefined,
    {
      init: {
  ...
  return (
    <div className="container mx-auto py-8 px-4 lg:px-24">
      <h1 className="text-xl md:text-2xl lg:text-3xl font-bold mb-8">
        Psyhologist Lists
      </h1>
          ...
          <Card key={psychologist.id} className="h-fit w-auto">
            ...
            <CardFooter className="flex justify-between items-center">
              ...
              <Link href={`/bookings/psychologist/${psychologist.id}`}>
                <Button size="sm">
                  Booking <CalendarHeart className="ml-2 w-4 h-4" />
                </Button>
              </Link>
            </CardFooter>
          </Card>
