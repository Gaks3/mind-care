...
users.get("/", mustBeAnAdmin(), async (c) => {
...
users.get("/:id", mustBeAuthenticated(), async (c) => {
...
users.patch(
  ...
  async (c) => {
    ...
    if ((id !== session!.id || data.role) && !hasRole(session!, UserRole.ADMIN))
      return c.json({ error: "Unauthorized" }, 401);
...
users.delete("/:id", mustBeAuthenticated(), async (c) => {
  const { id } = c.req.param();
  const session = c.get("session")!;
